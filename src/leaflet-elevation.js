(function (factory) {
	 ...
	 */
	const parseInt  = (property, id) => window.parseInt((typeof property === 'object' ? property[id] : property));
	 ...
	 */
	const iMax = (iVal, max) => (iVal > max ? iVal : max) || -Infinity; 
	const iMin = (iVal, min) => (iVal < min ? iVal : min) || +Infinity;
	const iAvg = (iVal, avg) => (iVal + (avg || 0)) / 2.0;
	const iSum = (iVal, sum) => iVal + (sum || 0);
		...
		_loadModules: function(handlers) {
			...
			return Promise.
				...
				.then((m) => {
					...
					each(handlers, h => this._registerHandler(h));
				});
		},
